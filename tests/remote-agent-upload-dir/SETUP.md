...
Implements REQUIREMENT-DESIGN-remote-agent-upload-dir.md and
REQUIREMENT-DESIGN-upload-download-dry-run.md. Directory uploads are
client-orchestrated fan-outs of the existing per-file chunked upload API; rejected
leaves must show no partial writes under `remoteDir`. Dry-run leaves snapshot
`serverHome` before/after CLI (`snapshotDataTree` / `assertTreeSnapshotUnchanged`).
...
func setUploadArgs(t *testing.T, req *Request, localPath, remotePath string) {
	t.Helper()
	setUploadArgsWithDryRun(t, req, localPath, remotePath, false)
}

func setUploadDryRunArgs(t *testing.T, req *Request, localPath, remotePath string) {
	t.Helper()
	setUploadArgsWithDryRun(t, req, localPath, remotePath, true)
}

func setUploadArgsWithDryRun(t *testing.T, req *Request, localPath, remotePath string, dryRun bool) {
	t.Helper()
