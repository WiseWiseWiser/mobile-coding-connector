...
1. `Method` is `POST`.
2. `Path` starts with `/api/cron-tasks/enable?`.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if !strings.HasPrefix(resp.Path, "/api/cron-tasks/enable?") {
		t.Fatalf("path = %q, want prefix %q", resp.Path, "/api/cron-tasks/enable?")
	}
