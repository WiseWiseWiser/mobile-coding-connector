...
4. Argv contains adjacent `-repeat` then `watch`.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if !argvHasToken(resp.Argv, "-batch") {
		t.Fatalf("watch build should keep -batch; got %v", resp.Argv)
	}
	if !argvHasAdjacent(resp.Argv, "-repeat", "watch") {
		t.Fatalf("watch build missing adjacent -repeat watch; got %v", resp.Argv)
	}
